...
public class ExecCodeMargeShellController {
    ...
    @GetMapping("select")
    public List<ShellVo.MargeVo> select() {
        List<GitAuditRecord> gitAuditRecordList = gitAuditRecordMapper.selectAll();
    ...
    public boolean submit(@RequestBody @Valid ShellDto.MargeDto margeDto) {
        ...
        gitAuditRecord.setSerialNumber(System.currentTimeMillis()).setServiceName(margeDto.getServiceName().trim())
                ...
                .setSubmitTime(System.currentTimeMillis()).setAuditTime(0L).setCreateTime(System.currentTimeMillis()).setExecLog("");
